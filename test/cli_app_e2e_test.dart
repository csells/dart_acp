...
void main() {
  group('CLI app e2e real adapters', tags: 'e2e', () {
    ...
    setUpAll(() async {
      ...
      final originalSettings = jsonDecode(
        File('test/test_settings.json').readAsStringSync(),
      );
      ...
      if (agentServers != null && agentServers['echo'] != null) {
        final echoConfig = Map<String, dynamic>.from(
          agentServers['echo'] as Map<String, dynamic>,
        );
        echoConfig['args'] = [
          File('test/agents/echo_agent.dart').absolute.path,
        ];
      ...
      final tempSettings = await Directory.systemTemp.createTemp(
        'test_settings_',
      );
    ...
    test('echo: --list-xxx flags in JSONL mode', () async {
      ...
      final hasSelectedAgent = jsonObjects.any(
        (m) => m['method'] == 'client/selected_agent',
      );
      final hasCaps = jsonObjects.any(
        (m) => m['method'] == 'client/capabilities',
      );
